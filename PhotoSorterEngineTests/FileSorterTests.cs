...
namespace PhotoSorterEngineTests
{
    public class FileReorderCalculatorTests : TestsBase
    {
        ...
        public void FileSorterPositiveTest()
        {
            ...
            var sourceFiles = fileEnumerator.EnumerateFiles(@"F:\Test", MediaType.All);
            var result = fileSorter.Calculate(sourceFiles,
                new SortParameters(@"F:\Photos", @"%YYYY%\%Comment%%YYYY%-%MM%-%DD%%Comment%", UseFileCreationDateIfNoExif: false));
